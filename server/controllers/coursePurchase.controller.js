...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `https://learning-management-system-peerlearn.onrender.com/course-progress/${courseId}`,
      cancel_url: `https://learning-management-system-peerlearn.onrender.com/course-detail/${courseId}`,
      metadata: {
        courseId: courseId,
        userId: userId,
      },
...
export const getCourseDetailWithPurchaseStatus = async (req, res) => {
  try {
      ...
      const userId = req.id; // Extracted from authenticated middleware
  ...
  try {
    ...
    if(!purchasedCourse){
      return res.status(404).json({
        message :"no course has been purchased yet",
        purchasedCourse:[]
      })
    }
    return res.status(200).json({
